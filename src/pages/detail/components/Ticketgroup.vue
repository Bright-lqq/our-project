<template>
	<div>
		<div class="ticket-group mp-border-bottom" v-for="Item in TicketgroupInfo" :key="Item.id">
			...
			<div class="ticket-type-info border-bottom">
				...
				<div class="ticket-type-price price">&yen<em class="price-num">{{Item.price}}</em>
					<span class="ticket-numword">起</span>
				</div>
				<i class="iconfont icon-down-trangle-copy-copy1 ticket-type-extend" @click="handleUpDownClick(Item.id)"></i>
			</div>
			<div class="ticket-default" id="ticket-default" v-if="showItem(Item.id)">
				<div class="ticket-item  border-top"  v-for="value in Item.list">
					<div class="ticket-main">
						<div>
							<h4 class="ticket-title-name">{{value.name2}}</h4>
							<h6 class="ticket-title ellipsis2">{{value.title2}}</h6>
					...
					<div class="ticket-side flexbox-layout border-left">
						<a href=""title="value.title2" class="ticket-link">
							<strong class="ticket-sale price">&yen<em class="price-num">{{value.price2}}</em></strong>
							<em class="ticket-btn promote-ablebtn">{{value.ablebtn}}</em>
						</a>
					</div>
				</div>
			</div>
